...
class BaseTrainer:
    ...
    def _build_train_pipeline(self):
        ...
        self.optimizer = self.build_optimizer(
            model=self.qat_model if self.qat_model is not None else self.model,
                ...
                try:
                    with autocast(self.amp):
                        ...
                        if self.qat_model is not None:
                            ...
                            teacher_preds = None
                            if self.teacher_model is not None and getattr(self.args, "qat_kd", False):
                                with torch.no_grad():
                                    teacher_preds = self.teacher_model(batch["img"])
                            loss, self.loss_items = unwrap_model(self.model).loss(
                                batch, preds, teacher_preds=teacher_preds
                            )
    ...
    def optimizer_step(self):
        ...
        grad_model = self.qat_model if self.qat_model is not None else self.model
        self.scaler.unscale_(self.optimizer)  # unscale gradients
        torch.nn.utils.clip_grad_norm_(grad_model.parameters(), max_norm=10.0)
    ...
    def validate(self):
        ...
        """
        if self.qat_model is not None and not getattr(self.args, "qat_validate", False):
            metrics = self.metrics.copy() if isinstance(self.metrics, dict) else {}
            fitness = self.best_fitness if self.best_fitness is not None else -float(self.loss.detach().cpu())
    ...
    def final_eval(self):
        ...
        if self.qat_model is not None:
            LOGGER.info("Skipping checkpoint-based final_eval for PT2E QAT model.")
            return
