...
class Detect(nn.Module):
    ...
    def forward_head(
        ...
        if concat_flag:
            ...
            scores = [
                (cls_head[i](x[i]).sigmoid() if _cs else cls_head[i](x[i])).view(bs, self.nc, -1)
    ...
    def forward(
        ...
        """Concatenates and returncos predicted bounding boxes and class probabilities."""
    ...
    def _inference(self, x: dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        return torch.cat(
            (dbox, x["scores"] if getattr(self, "cls_sigmoid_inplace", False) else x["scores"].sigmoid()), 1
        )
...
class OBB(Detect):
    ...
    def forward_head(
        ...
        # OBB loss consumes concatenated predictions, unlike the per-level detect output.
        del concat_flag
...
class OBB26(OBB):
    ...
    def forward_head(
        ...
        # Keep OBB predictions concatenated for its loss implementation.
        del concat_flag
...
class Pose(Detect):
    ...
    def forward_head(
        ...
        # Pose loss consumes concatenated predictions, unlike the per-level detect output.
        del concat_flag
        preds = super().forward_head(x, box_head, cls_head)
