...
SKIP_DATASET_HASH_ENV = "ULTRALYTICS_SKIP_DATASET_HASH"
...
class YOLODataset(BaseDataset):
    ...
    def get_labels(self) -> list[dict]:
        ...
        skip_dataset_hash = os.getenv(SKIP_DATASET_HASH_ENV, "").strip().lower() in {"1", "true", "yes", "on"}
        try:
            cache, exists = load_dataset_cache_file(cache_path), True  # attempt to load a *.cache file
            assert cache["version"] == DATASET_CACHE_VERSION  # matches current version
            if not skip_dataset_hash:
        ...
        if exists and LOCAL_RANK in {-1, 0}:
            ...
            if skip_dataset_hash:
                LOGGER.info(f"{self.prefix}Skipping dataset hash validation because {SKIP_DATASET_HASH_ENV}=1")
            if cache["msgs"]:
    ...
    def build_transforms(self, hyp: dict | None = None) -> Compose:
        ...
        if self.augment:
            ...
            new_shape = tuple(self.imgsz) if isinstance(self.imgsz, (tuple, list)) else (self.imgsz, self.imgsz)
