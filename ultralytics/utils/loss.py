...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk: int = 10, tal_topk2: int | None = None):  # model must be de-paralleled
        ...
        self.cls_sigmoid = getattr(model.model[-1], "cls_sigmoid_inplace", False)
        self.bce_score = nn.BCELoss(reduction="none") if self.cls_sigmoid else None
        self.hyp = h
        ...
        self.use_kd = bool(getattr(h, "qat_kd", False))
        self.kd_lambda = float(getattr(h, "qat_kd_lambda", 1.0))
        self.kd_temperature = float(getattr(h, "qat_kd_temperature", 4.0))
    ...
    def get_assigned_targets_and_loss(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> tuple:
        ...
        # Cls loss
        if getattr(self, "cls_sigmoid", False):
            loss[1] = (
                self.bce_score(pred_scores.clamp(1e-6, 1 - 1e-6), target_scores.to(dtype)).sum() / target_scores_sum
            )
        else:
    ...
    def kd_loss(
        ...
        teacher_preds: dict[str, torch.Tensor] | None,
    ) -> torch.Tensor:
        """Calculate KD loss between student and teacher raw detection outputs."""
        if not self.use_kd or teacher_preds is None:
            return torch.zeros((), device=self.device)

        temperature = max(self.kd_temperature, 1e-6)
        student_scores = student_preds["scores"].permute(0, 2, 1).contiguous().reshape(-1, self.nc)
        teacher_scores = teacher_preds["scores"].permute(0, 2, 1).contiguous().reshape(-1, self.nc)
        cls_kd = F.kl_div(
            F.log_softmax(student_scores / temperature, dim=-1),
            F.softmax(teacher_scores / temperature, dim=-1),
            reduction="batchmean",
        ) * (temperature**2)
        box_kd = F.mse_loss(student_preds["boxes"], teacher_preds["boxes"], reduction="mean")
        return self.kd_lambda * (cls_kd + box_kd)

    def __call__(
        self,
        preds: dict[str, torch.Tensor] | tuple[torch.Tensor, dict[str, torch.Tensor]],
        batch: dict[str, torch.Tensor],
        teacher_preds: dict[str, torch.Tensor] | tuple[torch.Tensor, dict[str, torch.Tensor]] | None = None,
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
        parsed_teacher_preds = None if teacher_preds is None else self.parse_output(teacher_preds)
        return self.loss(self.parse_output(preds), batch, teacher_preds=parsed_teacher_preds)
    ...
    def loss(
        ...
        if isinstance(preds.get("boxes"), list):  # e2e=False：training forward 返回 per-scale list，concat 成 tensor
            preds = {**preds, "boxes": torch.cat(preds["boxes"], dim=-1), "scores": torch.cat(preds["scores"], dim=-1)}
        batch_size = preds["boxes"].shape[0]
        loss, loss_detach = self.get_assigned_targets_and_loss(preds, batch)[1:]
        if self.use_kd:
            kd_loss = self.kd_loss(preds, teacher_preds)
            loss = torch.cat((loss, kd_loss.unsqueeze(0)))
            loss_detach = torch.cat((loss_detach, kd_loss.detach().unsqueeze(0)))
...
class E2EDetectLoss:
    ...
    def __call__(
        self, preds: Any, batch: dict[str, torch.Tensor], teacher_preds: Any = None
    ) -> tuple[torch.Tensor, torch.Tensor]:
        """Calculate the sum of the loss for box, cls and dfl multiplied by batch size."""
        preds = preds[1] if isinstance(preds, tuple) else preds
        t = None
        if teacher_preds is not None:
            t = teacher_preds[1] if isinstance(teacher_preds, tuple) else teacher_preds
        one2many = preds["one2many"]
        loss_one2many = self.one2many(one2many, batch, teacher_preds=(t["one2many"] if t is not None else None))
        one2one = preds["one2one"]
        loss_one2one = self.one2one(one2one, batch, teacher_preds=(t["one2one"] if t is not None else None))
