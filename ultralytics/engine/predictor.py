            ...
            for self.batch in self.dataset:
                ...
                with profilers[1]:
                    ...
                    if isinstance(qat_preds, dict):
                        qat_preds_infer = self.model.model.model._modules.get(det_id)._inference(qat_preds['one2one'])
                        qat_preds_infer = self.model.model.model._modules.get(det_id).postprocess(qat_preds_infer.permute(0, 2, 1), 300, 80)
                    else:
                        qat_preds_infer = self.model.model.model._modules.get(det_id)._inference(qat_preds)
                    preds = (qat_preds_infer, qat_preds)

                    # import onnxruntime as ort
                    # sess = ort.InferenceSession(self.args.onnxpath)
                    # ## for qat_onnx_model
                    # qat_preds = sess.run(None, {"x": im.cpu().numpy()})
                    # qat_preds[0] = torch.Tensor(qat_preds[0]).to(im)
                    # qat_preds[1] = torch.Tensor(qat_preds[1]).to(im)
                    # qat_preds[2] = torch.Tensor(qat_preds[2]).to(im)
                    # det_id = list(self.model.model.model._modules.keys())[-1]
                    # if isinstance(qat_preds, dict):
                    #     qat_preds_infer = self.model.model.model._modules.get(det_id)._inference(qat_preds['one2one'])
                    #     qat_preds_infer = self.model.model.model._modules.get(det_id).postprocess(qat_preds_infer.permute(0, 2, 1), 300, 80)
