...
class AAttn(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.area > 1:
            ...
            qkv = qkv.reshape(-1, N // self.area, C * 3)
            B, N, _ = qkv.shape
        q, k, v = (
            qkv.view(-1, N, self.num_heads, self.head_dim * 3)
            .permute(0, 2, 3, 1)
        ...
        if self.area > 1:
            ...
            x = x.reshape(-1, N * self.area, C)
            v = v.reshape(-1, N * self.area, C)
            B, N, _ = x.shape

        # x = x.reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        # v = v.reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        x = x.reshape(-1, H, W, C).permute(0, 3, 1, 2).contiguous()
        v = v.reshape(-1, H, W, C).permute(0, 3, 1, 2).contiguous()
